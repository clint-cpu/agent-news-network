...
vi.mock('../p2p.js', async () => {
  const actual = await vi.importActual<typeof import('../p2p.js')>('../p2p.js');
  return {
    ...actual,
...
function validatePublishKnowledge(args: any) {
...
function validateSearchKnowledge(args: any) {
  const { query } = args;
...
describe('MCP Tools', () => {
  ...
  it('publish_knowledge should reject empty title', () => {
    expect(() =>
      validatePublishKnowledge({ title: '', content: 'x', status: 'resolved' })
  ...
  it('publish_knowledge should reject oversized title', () => {
    ...
    expect(() =>
      validatePublishKnowledge({ title: longTitle, content: 'x', status: 'resolved' })
    ).toThrow('title must be a non-empty string of at most 512 characters');
  });

  it('publish_knowledge should reject invalid status', () => {
    expect(() =>
      validatePublishKnowledge({ title: 'Valid', content: 'Valid', status: 'unknown' })
    ).toThrow('status must be one of: resolved, partial, failed');
  });

  it('publish_knowledge should accept valid parameters', () => {
    const result: any = validatePublishKnowledge({ title: 'Hello', content: 'World', status: 'resolved', artifacts: [] });
    expect(result.content[0].text).toContain('Published: Hello');
  });

  it('search_knowledge should reject empty query', () => {
    expect(() =>
      validateSearchKnowledge({ query: '' })
    ).toThrow('query must be a non-empty string');
  });

  it('search_knowledge should return text content array', () => {
    const result: any = validateSearchKnowledge({ query: 'typescript' });
