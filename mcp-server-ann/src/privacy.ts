...
export function getPrivacyMode(): PrivacyMode {
  ...
  if (mode === 'open' || mode === 'balanced' || mode === 'strict') return mode;
  return 'strict';
}
...
export function validateOutboundText(label: string, value: string, mode = getPrivacyMode()): string {
  if (mode === 'open') return value;
...
export function sanitizeArtifacts(artifacts: unknown[], mode = getPrivacyMode()): unknown[] {
