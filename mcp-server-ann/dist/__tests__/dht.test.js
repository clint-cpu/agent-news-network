...
import { dhtPutContent, dhtGetContent, dhtIndexKeyword, dhtQueryKeyword, isExpired, extractKeywords, } from '../p2p.js';
...
describe('DHT Read/Write', () => {
    ...
    it('should put and get content', async () => {
        ...
        await dhtPutContent(mockNode, cid, item, expiresAt);
        const result = await dhtGetContent(mockNode, cid);
        expect(result).toBeDefined();
        expect(result.title).toBe('Test');
        expect(result.expires_at).toBe(expiresAt);
    });
    it('should return null for non-existent key', async () => {
        const result = await dhtGetContent(mockNode, 'nonexistent');
        expect(result).toBeNull();
    });
    it('should index and query keywords', async () => {
        const cid = 'cid-1';
        await dhtIndexKeyword(mockNode, 'typescript', cid);
        await dhtIndexKeyword(mockNode, 'nodejs', cid);
        ...
        const tsCids = await dhtQueryKeyword(mockNode, 'typescript');
        const nodeCids = await dhtQueryKeyword(mockNode, 'nodejs');
        expect(tsCids).toContain(cid);
        expect(nodeCids).toContain(cid);
    });
    it('should dedupe cids in keyword index', async () => {
        const cid = 'cid-1';
        await dhtIndexKeyword(mockNode, 'rust', cid);
        await dhtIndexKeyword(mockNode, 'rust', cid);
        ...
        const cids = await dhtQueryKeyword(mockNode, 'rust');
    ...
    it('should clean up expired entries on query', async () => {
        ...
        // Also index it
        await dhtIndexKeyword(mockNode, 'oldstuff', cid);
        const result = await dhtGetContent(mockNode, cid);
        expect(result).toBeNull();
        const cids = await dhtQueryKeyword(mockNode, 'oldstuff');
