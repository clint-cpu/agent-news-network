...
export async function setupPubsubHandlers(nodeInstance, mode) {
    nodeInstance.services.pubsub.addEventListener('message', async (evt) => {
 ...
 */
export async function estimateNetworkSize() {
    if (!node)
        return 10;
...
export async function encodeErasure(dataBuffer, networkSize) {
    let dataShards = 10;
    let parityShards = 5;
    if (networkSize < 100) {
        dataShards = 4;
...
export async function decodeErasure(shards, dataShards, parityShards, shardSize, originalLength) {
 ...
 */
function buildCapabilityCard(identity) {
    return {
        type: "ann-capability",
 ...
 */
export async function dhtPutContent(node, cid, item, expiresAt) {
 ...
 */
export async function dhtIndexKeyword(node, keyword, cid) {
    if (!node.services.dht)
        return;
 ...
 */
export async function dhtRemoveFromIndex(node, title, content, cid) {
    if (!node)
        return;
 ...
 */
export async function indexToDHT(node, cid, title, content, item, expiresAt) {
    const keywords = extractKeywords(title, content);
 ...
 */
export function repKey(pubkeyHex) {
 ...
 */
export async function getReputation(node, pubkeyHex) {
    if (!node.services.dht) {
 ...
 */
export async function updateReputation(node, pubkeyHex, domainMatch, declaredDomains) {
    if (!node.services.dht) {
        return { total_score: 0, event_count: 0, domain_scores: {}, last_updated: 0 };
    }
 ...
 */
export function reputationWeight(ledger) {
    if (ledger.event_count === 0)
        return 0.5; // unknown
    if (ledger.event_count <= 5)
        return 0.8; // new participant
    if (ledger.event_count <= 20)
 ...
 */
export function contentMatchesDeclaredDomain(title, content, declaredDomains) {
 ...
 */
export async function dhtGetContent(node, cid) {
    if (!node.services.dht)
        return null;
    const key = new TextEncoder().encode(`ann:content:${cid}`);
    try {
 ...
 */
export async function dhtQueryKeyword(node, keyword) {
    if (!node.services.dht)
        return [];
    const key = new TextEncoder().encode(`ann:index:${keyword}`);
    let cids = [];
 ...
 */
export async function dhtSweepExpired(node) {
    let deleted = 0;
